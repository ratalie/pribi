<script setup lang="ts">
  ...
  // Auto-expandir secciones e items que contienen la ruta activa
  const expandActiveItems = () => {
    navigationSections.forEach((section) => {
      const hasActiveItem = section.items.some((item) => {
        if (item.href && isActive(item.href)) return true;
        if (item.submenuItems) {
          return item.submenuItems.some((subItem) => subItem.href && isActive(subItem.href));
        }
        return false;
      });
      ...
      if (hasActiveItem) {
        ...
        section.items.forEach((item) => {
          if (item.submenuItems) {
            const hasActiveSubItem = item.submenuItems.some(
  ...
  // Watch ruta para auto-expandir cuando cambia
  watch(
    () => route.path,
    () => {
      ...
      <SidebarContent class="probo-sidebar-content">
        ...
        <template v-else>
          <div class="probo-collapsed-content">
            ...
            >
              <template v-if="canViewModule(section.id)">
                <!-- Nivel 1: Sección (Solo Nombre, SIN icono cuando está contraído) -->
                <button
