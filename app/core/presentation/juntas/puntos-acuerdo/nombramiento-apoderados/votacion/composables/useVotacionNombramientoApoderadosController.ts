 ...
 * - Generación de preguntas desde candidatos (solo los que tienen isCandidate: true)
 * - Guardado de votos para cada item
 */
export function useVotacionNombramientoApoderadosController() {
  const route = useRoute();
  const votacionStore = useVotacionNombramientoApoderadosStore(); // ✅ Store dedicado
  ...
  async function loadData() {
    try {
      ...
      if (nombramientoStore.apoderadosCandidatos.length === 0) {
        console.log(
          "[DEBUG][VotacionNombramientoApoderadosController] Cargando candidatos desde backend..."
        );
        try {
          await nombramientoStore.loadApoderadosDesignados(societyId.value, flowId.value);
          console.log(
            "[DEBUG][VotacionNombramientoApoderadosController] Candidatos cargados:",
            nombramientoStore.apoderadosCandidatos.length
          );
        } catch (error: any) {
          console.warn(
            "[DEBUG][VotacionNombramientoApoderadosController] Error al cargar candidatos:",
            error
          );
      ...
      try {
        ...
        console.log("[DEBUG][VotacionNombramientoApoderadosController] Votación cargada:", {
        ...
        let candidatosFiltrados = nombramientoStore.apoderadosCandidatos; // ✅ Ya filtra por isCandidate: true

        // ✅ Filtrar Gerente General (se maneja en su propia votación)
        const gerenteGeneralClassId = nombramientoStore.getGerenteGeneralClassId();
        if (gerenteGeneralClassId) {
          candidatosFiltrados = candidatosFiltrados.filter(
            (c) => c.attorneyClassId !== gerenteGeneralClassId
          );
        }
        ...
        if (
          ...
          console.warn(
            "[DEBUG][VotacionNombramientoApoderadosController] ⚠️ Hay candidatos marcados pero no hay items de votación. " +
          ...
          if (votacionStore.items.length === 0) {
            console.error(
              "[DEBUG][VotacionNombramientoApoderadosController] ❌ Los items de votación no se crearon automáticamente. " +
        ...
        if (error.statusCode === 404 || error.status === 404) {
          console.log(
            "[DEBUG][VotacionNombramientoApoderadosController] No hay votación existente (404), se creará al guardar"
          );
        } else {
          console.error(
            "[Controller][VotacionNombramientoApoderados] Error al cargar votación:",
            error
          );
      ...
      console.log(
        "[DEBUG][VotacionNombramientoApoderadosController] Carga de datos completada:",
        {
          hasVotacion: votacionStore.hasVotacion,
          itemsCount: votacionStore.items.length,
  ...
  /**
   * ✅ Items filtrados (excluyendo Gerente General)
   * Se usa para mantener consistencia entre preguntas e items
   */
  const itemsFiltrados = computed(() => {
    ...
    // ✅ Filtrar items del Gerente General (se maneja en su propia votación)
    const gerenteGeneralClassId = nombramientoStore.getGerenteGeneralClassId();
    const snapshot = snapshotStore.snapshot;
    ...
    // Obtener todos los personaIds posibles del Gerente General desde snapshot
    let gerenteGeneralPersonaIds = new Set<string>();

    // 1. Buscar en snapshot.gerenteGeneral
    if (snapshot?.gerenteGeneral?.persona?.id) {
      gerenteGeneralPersonaIds.add(snapshot.gerenteGeneral.persona.id);
    }

    // 2. Buscar en snapshot.attorneys donde claseApoderadoId === gerenteGeneralClassId
    if (snapshot?.attorneys) {
      snapshot.attorneys
        .filter((att) => att.claseApoderadoId === gerenteGeneralClassId)
        .forEach((att) => {
          if (att.persona?.id) {
            gerenteGeneralPersonaIds.add(att.persona.id);
          }
        });
    }

    // 3. También buscar en apoderados designados que tengan attorneyClassId del Gerente General
    nombramientoStore.apoderadosDesignados
      .filter((apod) => apod.attorneyClassId === gerenteGeneralClassId)
      .forEach((apod) => {
        if (apod.person?.id) {
          gerenteGeneralPersonaIds.add(apod.person.id);
        }
      });

    // Buscar el personaId de cada item y verificar si corresponde al Gerente General
    const filtrados = itemsOrdenados.filter((item) => {
      // El item puede tener personaId (según el JSON del backend)
      const personaId = item.personaId;
      if (!personaId) {
        // Si no tiene personaId, no podemos filtrarlo, lo dejamos pasar
        return true;
      }

      // ✅ Verificar si es el Gerente General comparando personaId con todos los IDs posibles
      if (gerenteGeneralPersonaIds.has(personaId)) {
        console.log(
          `[DEBUG][VotacionNombramientoApoderadosController] ✅ Filtrando item del Gerente General (personaId encontrado):`,
          {
            itemId: item.id,
            personaId,
            label: item.label,
      ...
      // Si no es el Gerente General, incluirlo
      return true;
    });

    console.log(
      "[DEBUG][VotacionNombramientoApoderadosController] ✅ Items filtrados (excluyendo Gerente General):",
      {
        itemsAntes: itemsOrdenados.length,
        itemsDespues: filtrados.length,
        gerenteGeneralClassId,
        gerenteGeneralPersonaIds: Array.from(gerenteGeneralPersonaIds),
      }
    );
  ...
  const preguntas = computed(() => {
    // ✅ Prioridad 1: Usar items filtrados de la sesión de votación del backend
    const items = itemsFiltrados.value;
    if (items.length > 0) {
      const preguntasFromItems = items.map((item) => item.label);

      console.log(
        "[DEBUG][VotacionNombramientoApoderadosController] ✅ Preguntas desde sesión:",
        {
          itemsCount: votacionStore.items.length,
          itemsFiltradosCount: items.length,
    ...
    if (candidatosFiltrados.length > 0) {
      ...
      console.log(
        "[DEBUG][VotacionNombramientoApoderadosController] ✅ Preguntas desde candidatos:",
        {
          candidatosTotal: nombramientoStore.apoderadosDesignados.length,
    ...
    console.warn(
      "[DEBUG][VotacionNombramientoApoderadosController] ⚠️ No hay preguntas disponibles. Retornando array vacío."
    );
    return [];
  });
  ...
  const mensajeAprobacion = computed(() => {
    return "el nombramiento del apoderado.";
  });
  ...
  function sincronizarVotosConVotantesActuales() {
    ...
    votacionStore.items.forEach((item) => {
      ...
      if (votosEliminados > 0) {
        console.warn(
          `[DEBUG][VotacionNombramientoApoderadosController] ⚠️ Item ${item.id}: Se eliminaron ${votosEliminados} votos de accionistas que ya no tienen derecho a voto`
        );
      }
    });
  }
   ...
   *
   * @param preguntaIndex - Índice del item filtrado (pregunta)
  ...
  function getVoto(preguntaIndex: number, accionistaId: string): VoteValue | null {
    // ✅ Usar items filtrados para obtener el item correcto
    const items = itemsFiltrados.value;
    const item = items[preguntaIndex];
    if (!item) return null;

    // Buscar el voto en el item
  ...
  function getVotoForComponent(accionistaId: string): VoteValue | null {
    // Para nombramiento de apoderados, siempre usar el primer item (pregunta)
  ...
  async function setVoto(preguntaIndex: number, accionistaId: string, valor: VoteValue) {
    ...
    if (!votacionStore.sesionVotacion) {
      ...
      votacionStore.sesionVotacion = {
        id: sessionId,
        contexto: VoteContext.DESIGNACION_APODERADOS,
        modo: VoteMode.SIMPLE,
        items: preguntasValue.map((pregunta, index) => ({
          id: votacionStore.generateUuid(),
          orden: index,
          label: pregunta,
          descripción: `Votación sobre el nombramiento del apoderado ${index + 1}`,
    ...
    // ✅ Obtener el item real desde items filtrados para usar su ID/índice correcto
    const items = itemsFiltrados.value;
    const item = items[preguntaIndex];
    if (!item) {
      console.warn(
        `[VotacionNombramientoApoderadosController] Item no encontrado para índice ${preguntaIndex}`
      );
      return;
    }

    // Buscar el índice real del item en el store
    const itemIndexReal = votacionStore.items.findIndex((it) => it.id === item.id);
    if (itemIndexReal === -1) {
      console.warn(
        `[VotacionNombramientoApoderadosController] No se encontró el índice real del item ${item.id}`
      );
      return;
    }

    // ✅ Usar el store dedicado para agregar/actualizar voto con el índice real
  ...
  async function cambiarTipoAprobacionItem(itemIndex: number, tipo: "unanimidad" | "mayoria") {
    ...
    if (!votacionStore.sesionVotacion) {
      ...
      // ✅ Obtener el item real desde items filtrados para usar su índice correcto
      const items = itemsFiltrados.value;
      const item = items[itemIndex];
      if (!item) {
        console.warn(
          `[VotacionNombramientoApoderadosController] Item no encontrado para índice ${itemIndex}`
        );
        return;
      }
      ...
      // Actualizar tipoAprobacion en el item específico usando el store con el índice real
  ...
  async function guardarVotacion() {
    console.log(
      "[DEBUG][VotacionNombramientoApoderadosController] guardarVotacion() ejecutado"
    );
    ...
    try {
      await votacionStore.loadVotacion(societyId.value, flowId.value);
      console.log("[DEBUG][VotacionNombramientoApoderadosController] GET ejecutado, estado:", {
        hasVotacion: votacionStore.hasVotacion,
        itemsCount: votacionStore.items.length,
      ...
      if (error.statusCode === 404 || error.status === 404) {
        console.log(
          "[DEBUG][VotacionNombramientoApoderadosController] No hay votación existente (404), se creará"
        );
      } else {
        console.error(
          "[Controller][VotacionNombramientoApoderados] Error al cargar votación antes de guardar:",
          error
        );
    ...
    if (!votacionStore.sesionVotacion) {
      ...
      votacionStore.sesionVotacion = {
        id: sessionId,
        contexto: VoteContext.DESIGNACION_APODERADOS, // ✅ Contexto explícito
        modo: VoteMode.SIMPLE,
        items: preguntasValue.map((pregunta, index) => ({
          id: votacionStore.generateUuid(),
          orden: index,
      ...
      if (votacionStore.sesionVotacion.contexto !== VoteContext.DESIGNACION_APODERADOS) {
        console.warn(
          "[DEBUG][VotacionNombramientoApoderadosController] ⚠️ Contexto incorrecto en sesión existente, corrigiendo...",
          {
            contextoActual: votacionStore.sesionVotacion.contexto,
            contextoCorrecto: VoteContext.DESIGNACION_APODERADOS,
          }
        );
        votacionStore.sesionVotacion.contexto = VoteContext.DESIGNACION_APODERADOS;
      }
    ...
    // ✅ 3. Usar items filtrados (sin Gerente General)
    const items = itemsFiltrados.value;
    ...
    items.forEach((item, index) => {
      ...
      if (tipoAprobacion === VoteAgreementType.APROBADO_POR_TODOS) {
        ...
        if (item.votos.length === 0) {
          console.warn(
            `[DEBUG][VotacionNombramientoApoderadosController] ⚠️ Item ${index} no tiene votos para mayoría`
          );
        }
      }
    });
    ...
    if (!votacionStore.hasVotacion) {
      ...
      if (candidatosFiltrados.length === 0) {
        throw new Error("No hay apoderados seleccionados para nombramiento.");
      }

      // El backend debería haber creado la sesión automáticamente
      // Intentar recargar una vez más
      console.warn(
        "[DEBUG][VotacionNombramientoApoderadosController] ⚠️ Sesión no existe. Recargando..."
      );
      await votacionStore.loadVotacion(societyId.value, flowId.value);
    ...
    try {
      // Actualizar todos los items filtrados existentes (solo votos, no crear items)
      ...
      console.log(
        "[DEBUG][VotacionNombramientoApoderadosController] Guardado completado exitosamente"
      );

      // ✅ 6. Actualizar estados de candidatos según resultado de votación
      console.log(
        "[DEBUG][VotacionNombramientoApoderadosController] Actualizando estados de candidatos..."
      );

      // ✅ Usar apoderadosCandidatos (ya filtra por isCandidate: true) y mapear con items
      let candidatosFiltrados = nombramientoStore.apoderadosCandidatos;
      ...
      for (let i = 0; i < items.length; i++) {
        ...
        if (!item) {
          console.warn(
            `[Controller][VotacionNombramientoApoderados] No hay item para índice ${i}`
          );
          continue;
        }
        ...
        if (!candidato) {
          console.warn(
            `[Controller][VotacionNombramientoApoderados] No hay candidato para item ${i}`
          );
          continue;
        }

        // ✅ El id del registro de designación ES el designationId que se necesita
        const designationId = candidato.id;
        ...
        if (totalAcciones === 0) {
          console.warn(
            `[Controller][VotacionNombramientoApoderados] Total acciones es 0 para item ${i}`
          );
          continue;
        }
        ...
        console.log(
          `[Controller][VotacionNombramientoApoderados] Apoderado ${designationId}: ${porcentajeAFavor.toFixed(
            2
          )}% a favor → ${estado}`
        );
      }
      ...
      console.log(
        "[DEBUG][VotacionNombramientoApoderadosController] Estados actualizados exitosamente"
      );
    } catch (error: any) {
      console.error(
        "[Controller][VotacionNombramientoApoderados] Error al guardar votación:",
        error
      );
      throw error;
    }
  ...
  return {
    ...
    itemsFiltrados, // ✅ Exponer items filtrados para que la vista lo use
