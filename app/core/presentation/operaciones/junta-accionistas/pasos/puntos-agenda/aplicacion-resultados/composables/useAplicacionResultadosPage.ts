/**
 * Composable principal para la página de Aplicación de Resultados
 *
...
import { useAplicacionResultadosController } from "./useAplicacionResultadosController";
import { useAplicacionResultadosInitialization } from "./useAplicacionResultadosInitialization";

export function useAplicacionResultadosPage() {
  // Obtener IDs de la ruta
  const { societyId, flowIdNumber } = useJuntasRouteParams();

  // Controller para el botón "Siguiente"
  const { handleNext } = useAplicacionResultadosController();

  // Inicialización de la vista
  const { initialize } = useAplicacionResultadosInitialization();
