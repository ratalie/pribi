...
export function useVotacionAplicacionResultadosController() {
  ...
  function mapearVotantesDesdeSnapshot() {
    ...
    if (!snapshot) {
      console.warn(
        "[DEBUG][VotacionAplicacionResultadosController] No hay snapshot disponible para mapear votantes"
      );
      return [];
    }
  ...
  const votantes = computed(() => {
    ...
    console.log(
      "[DEBUG][VotacionAplicacionResultadosController] Votantes mapeados desde snapshot:",
      votantesMapeados
    );
    votantesMapeados.forEach((v, i) => {
      console.log(`[DEBUG][VotacionAplicacionResultadosController] Votante ${i}:`, {
        id: v.id,
