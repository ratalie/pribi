...
export const useLoadDataFlowCC = () => {
  ...
  const getParticipants = async () => {
    const url = `/society-profile/${societyId.value}/register-assembly/${flowId.value}/credit-capitalization/participants`;
  ...
  const getContributions = async () => {
    const url = `/society-profile/${societyId.value}/register-assembly/${flowId.value}/credit-capitalization/contributions`;
    const response = await loadDataFlow(url);
    return response;
  };
  ...
  const votacionAprobada = computed((): boolean => {
    ...
    // Determinar tipo de acuerdo (capitalizacion-creditos es CALIFICADO)
    const puntoId = "capitalizacion-creditos";
  ...
  onMounted(async () => {
    try {
      ...
      console.error("[UseLoadDataFlowCC] Error al cargar datos:", error);
    } finally {
