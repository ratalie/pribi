...
export function useVotacionAuditoresExternosController() {
  ...
  function mapearVotantesDesdeSnapshot() {
    ...
    if (!snapshot) {
      console.warn(
        "[DEBUG][VotacionAuditoresExternosController] No hay snapshot disponible para mapear votantes"
      );
      return [];
    }
  ...
  const votantes = computed(() => {
    ...
    console.log(
      "[DEBUG][VotacionAuditoresExternosController] Votantes mapeados desde snapshot:",
      votantesMapeados
    );
    votantesMapeados.forEach((v, i) => {
      console.log(`[DEBUG][VotacionAuditoresExternosController] Votante ${i}:`, {
        id: v.id,
