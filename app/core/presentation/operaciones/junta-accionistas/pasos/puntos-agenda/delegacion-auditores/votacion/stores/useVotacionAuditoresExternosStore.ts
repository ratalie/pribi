...
/**
 * Store para gestionar Votaciones de Designación de Auditores Externos
 *
 * ⚠️ IMPORTANTE: Usa Option API de Pinia (NO Composition API)
 */
export const useVotacionAuditoresExternosStore = defineStore("votacionAuditoresExternos", {
  state: () => ({
  ...
  getters: {
    ...
    /**
     * Obtiene el item de votación (solo hay 1 para designación de auditores)
     */
     ...
     *
     * @param puntoId - ID del punto de agenda (ej: "delegacion-auditores")
  ...
  actions: {
    ...
    async loadVotacion(societyId: number, flowId: number) {
      ...
      try {
        ...
        this.sesionVotacion = await useCase.execute(
          societyId,
          flowId,
          VoteContext.DESIGNACION_AUDITORES
        );
        ...
        console.error("[Store][VotacionAuditoresExternos] Error al cargar:", error);
        throw error;
      }
    },
    ...
    async createVotacion(
      ...
      try {
        ...
        console.log("[Store][VotacionAuditoresExternos] Creando sesión con datos:", {
        ...
        console.error("[Store][VotacionAuditoresExternos] Error al crear:", error);
        throw error;
      }
    },
    ...
    async addVote(
      ...
      try {
        ...
        await useCase.execute(societyId, flowId, VoteContext.DESIGNACION_AUDITORES, [
          {
            accion: "updateVote",
            itemId: item.id,
            votos: [
        ...
        console.error("[Store][VotacionAuditoresExternos] Error al agregar voto:", error);
        throw error;
      }
    },
    ...
    async updateVote(
      ...
      try {
        ...
        console.error("[Store][VotacionAuditoresExternos] Error al actualizar voto:", error);
        throw error;
      }
    },
    ...
    async updateItemConVotos(
      ...
      try {
        ...
        console.log(
          "[Store][VotacionAuditoresExternos] Actualizando item con votos:",
          JSON.stringify(itemPayload, null, 2)
        );

        await useCase.execute(societyId, flowId, VoteContext.DESIGNACION_AUDITORES, [itemPayload]);
        ...
        console.error(
          "[Store][VotacionAuditoresExternos] Error al actualizar item con votos:",
          error
        );
        throw error;
      }
