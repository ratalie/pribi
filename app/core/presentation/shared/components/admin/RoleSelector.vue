<script setup lang="ts">
  ...
  const selectedRole = computed({
    ...
    set: (value: SimpleRole) => {
      ...
      // Solo emitir update:modelValue - el store manejará los permisos por defecto
  ...
  const mapBackendRoleToSimpleRole = (
  ...
  const currentUserSimpleRole = computed(() =>
    mapBackendRoleToSimpleRole(currentUserRoleName.value)
  );
  ...
  const allRoles: Array<{
  ...
  }> = [
    {
      ...
      description:
        "Puede gestionar todos los usuarios del estudio, incluyendo Administradores.",
      icon: Shield,
    ...
    <div class="role-selector-grid">
      ...
      >
        <div
          ...
          :style="{
            backgroundColor: selectedRole === role.value ? role.color : 'var(--bg-muted)',
          }"
        >
          <component
            :is="role.icon"
            class="role-icon"
            :style="{
              color: selectedRole === role.value ? 'white' : 'var(--text-muted)',
            }"
          />
        </div>
        ...
        <div class="role-card-content">
          <h3
            ...
            :style="{
              color: selectedRole === role.value ? role.color : 'var(--text-primary)',
              fontFamily: 'var(--font-primary)',
              fontWeight: 600,
            }"
          >
