<template>
  <div class="container mx-auto p-6 max-w-6xl">
    <div class="mb-8">
      <h1 class="text-3xl font-bold mb-2">Test: Flow Junta de Accionistas</h1>
      <p class="text-muted-foreground">
        Visualización y testing del FlowConfig de Junta de Accionistas
      </p>
    </div>
      ...
      <div class="border rounded-lg p-6">
        ...
        <dl class="space-y-2">
          ...
          <div>
            ...
            <dd class="text-sm">{{ flowConfig.items.length }} (Nivel 0)</dd>
          </div>
          <div>
    ...
    <!-- FlowItems List (Nivel 0) -->
    <div class="mt-8 border rounded-lg p-6">
      <h2 class="text-xl font-semibold mb-4">FlowItems (Nivel 0)</h2>
      <div class="space-y-4">
        <div
          v-for="item in flowConfig.items"
          :key="item.identity.id"
          class="border-l-4 border-blue-500 pl-4 py-2"
        >
          <div class="font-semibold">{{ item.identity.label }}</div>
          ...
          <div
            v-if="item.hierarchy.children && item.hierarchy.children.length > 0"
            class="text-sm text-muted-foreground"
          >
            Children: {{ item.hierarchy.children.length }}
          </div>
        </div>
      </div>
    </div>
...
<script setup lang="ts">
  ...
  definePageMeta({
    title: "Test: Junta Accionistas Flow",
    description: "Testing page for Junta de Accionistas FlowConfig",
  });
</script>
