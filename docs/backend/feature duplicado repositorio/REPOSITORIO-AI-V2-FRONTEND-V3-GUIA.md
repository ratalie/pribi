...
Verifica si un documento con un nombre específico ya existe en una carpeta. Útil para detectar duplicados antes de subir nuevos documentos.
...
const checkDocumentDuplicate = async (
  ...
  return result.data; // { exists: true/false, document: {...} | null }
};
```
...
```typescript
// Antes de subir un documento, verificar si ya existe
const fileName = 'acta-junta-universal.docx';
...
if (checkResult.exists) {
  ...
  if (userChoice === 'Crear nueva versión') {
    // Subir como nueva versión del documento existente
    await uploadNewVersion(checkResult.document.documentCode, file);
  }
...
- ✅ **Búsqueda exacta**: El nombre debe coincidir exactamente (case-sensitive)
- ✅ **Búsqueda recursiva**: Busca en la carpeta y todas sus subcarpetas
- ✅ **Última versión**: Si existe, retorna información de la última versión del documento
- ✅ **Performance**: Consulta optimizada, respuesta rápida
