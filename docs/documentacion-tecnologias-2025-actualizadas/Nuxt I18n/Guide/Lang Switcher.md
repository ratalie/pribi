---
title: Lang Switcher
source: https://i18n.nuxtjs.org/docs/guide/lang-switcher
author:
  - "[[@nuxtjs/i18n]]"
published:
created: 2025-10-15
...
To work around the issue, you can set the option [`skipSettingLocaleOnNavigate`](https://i18n.nuxtjs.org/docs/api/options#skipsettinglocaleonnavigate) to `true` and handle setting the locale yourself from a `onBeforeEnter` transition hook defined in a plugin.
...
In contrast to Vue i18n you should not directly set `locale`, switch language by using [`setLocale()`](https://i18n.nuxtjs.org/docs/api/vue-i18n#setlocale) or navigating to a route returned by [`switchLocalePath()`](https://i18n.nuxtjs.org/docs/composables/use-switch-locale-path). This loads translations, triggers hooks and updates the locale cookie if used.[Lazy-load translations](https://i18n.nuxtjs.org/docs/guide/lazy-load-translations)
