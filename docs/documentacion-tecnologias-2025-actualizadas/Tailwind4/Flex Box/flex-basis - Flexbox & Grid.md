...
Use the `basis-[<value>]` syntaxto set the basis based on a completely custom value:
...
Prefix a `flex-basis` utilitywith a breakpoint variant like `md:` to only apply the utility at mediumscreen sizes and above:
...
Use the `--container-*` theme variables to customize the fixed-width basisutilities in your project:
