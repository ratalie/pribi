...
All schema & checks are converted to their closest JSON Schema equivalent. Some types have no analog and cannot be reasonably represented. See the [`unrepresentable`](https://zod.dev/#unrepresentable) section below for more information on handling these cases.
...
When converting to JSON Schema in `"input"` mode, `additionalProperties` is not set. See the [`io` docs](https://zod.dev/#io) for more information.
