...
export const useDescargarController = () => {
  ...
  const loadDocuments = async () => {
    try {
      ...
      const documentos = await useCase.execute(societyId.value, flowId.value);
...
const saveMeetingDetails = async (meetingId: string, data: any) => {
  try {
    const response = await $fetch(
      `/api/v2/society-profile/${societyId}/flow/${meetingId}/meeting-details`,
      {
        method: "PUT",
        body: {
...
export interface MeetingDetailsRepository {
  save(societyId: string, flowId: string, details: MeetingDetails): Promise<MeetingDetails>;

  get(societyId: string, flowId: string): Promise<MeetingDetails | null>;
}
```
...
export const useDetallesStore = defineStore("detalles", {
  ...
  actions: {
    async saveDetails(societyId: string, flowId: string, dto: MeetingDetailsDto) {
      this.loading = true;
      try {
...
2. Usar desde Use Case:
   ```typescript
   export class GenerateDocumentsUseCase {
     constructor(private adapter: DocumentGeneratorV2Adapter) {}
