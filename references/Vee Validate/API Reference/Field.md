---
title: Field
source: https://vee-validate.logaretm.com/v4/api/field/
author:
published:
created: 2025-10-21
...
```
vue<Field name="field" as="select">
...
```
vue<Field v-slot="{ value }" name="drink" as="select" multiple>
...
```
vue<Field name="field" as="my-text-field" />
```

tip

For validation to work, the rendered tag with `as` prop must conform to the events that the `Field` component listens for, you can view a list of these in the [Validation Behavior](https://vee-validate.logaretm.com/v4/guide/components/validation#validation-behavior)
...
```
vue<Field name="password" v-slot="{ field }">
...
This is because the file input doesn’t work with two-way binding since you cannot really force pick a file from a user’s device. Custom components may emit different events and may have additional requirements for two-way binding to work. You can check the [UI Libraries](https://vee-validate.logaretm.com/v4/examples/ui-libraries) section for a few examples on how to do that.
...
```
vue<!-- DONT: ⛔️  v-model on input tag -->
...
```
vue<button @click="resetField({ value: 'new value' })">Reset</button>
```
...
Validates the field by default unless explicitly [specified by validation triggers](https://vee-validate.logaretm.com/v4/guide/components/validation#customizing-validation-triggers). Can be used as an event handler to bind on the field. If the passed argument isn’t an event object it will be used as the new value for that field.
...
[Edit This Page on GitHub](https://github.com/logaretm/vee-validate/edit/main/docs/src/pages/api/field.mdx)
