---
title: useForm
source: https://vee-validate.logaretm.com/v4/api/use-form/
author:
published:
created: 2025-10-21
...
Alternatively, you can use a [typed schema](https://vee-validate.logaretm.com/v4/guide/composition-api/typed-schema/) to infer the form types from the validation schema if you are using yup or zod.
...
```
tssetFieldValue('age', 5); // ⛔️ TypeScript error
...
```
jsconst { setFieldTouched } = useForm();
...
`handleSubmit` contains a `withControlled` function that you can use to only submit fields controlled by `useField` or `useFieldModel`. Read the [guide](https://vee-validate.logaretm.com/v4/guide/composition-api/handling-forms) for more information.
...
```
tstype TouchedFlags = { [k: string]: boolean };
...
```
jsconst { values, resetForm } = useForm({
...
[Edit This Page on GitHub](https://github.com/logaretm/vee-validate/edit/main/docs/src/pages/api/use-form.mdx)
